...
const STAGES=[
  { part:"rBooster", tag:"SRB SEP · RIGHT BOOSTER", sep:"T+02:12 · STAGE 01",
    title:"About Me",
    type:"I'm Rumen Vasilev — a software engineer based in Sutton, London, working across cybersecurity, AI and robotics, with my mission set on the space industry. I like building software that has to hold up: secure, tested, and reliable in demanding environments.",
    extra:`<div class="tags"><span class="tag2">Software Engineer</span><span class="tag2">Cybersecurity</span><span class="tag2">AI &amp; Robotics</span><span class="tag2">Space</span></div>` },
...
function sizeStars(){
  ...
  for(let i=0;i<n;i++)stars.push({x:Math.random()*innerWidth,y:Math.random()*innerHeight,
...
function flareTexture(){
  ...
  g.strokeStyle=grad;
  for(let a=0;a<4;a++){ g.save(); g.rotate(a*Math.PI/4); g.lineWidth=a%2?1.4:2.4;
...
function makeStar(hue){
  ...
  grp.add(core);
  // soft glow
  const glow=new THREE.Sprite(new THREE.SpriteMaterial({map:glowTexture(),color:col,
    transparent:true,blending:THREE.AdditiveBlending,depthWrite:false}));
  glow.scale.setScalar(3.4); grp.add(glow);
  // sharp cross flare
  const flare=new THREE.Sprite(new THREE.SpriteMaterial({map:flareTexture(),color:0xffffff,
...
function typeIn(text,cls,speed){
  return new Promise(res=>{
    ...
    const iv=setInterval(()=>{ msgEl.textContent=text.slice(0,++i);
      if(i>=text.length){clearInterval(iv);res();} }, speed||58);
...
function typeOut(speed){
  return new Promise(res=>{
    ...
    const iv=setInterval(()=>{ msgEl.textContent=t.slice(0,--i);
      if(i<=0){clearInterval(iv);msgEl.className="";res();} }, speed||26);
    timers.push({clear:()=>clearInterval(iv)});
  });
}
function showMsg(text,cls){ msgEl.className=(cls||""); msgEl.textContent=text; }
function fadeOut(){
  return new Promise(res=>{ msgEl.classList.add("fade-out");
...
async function startMission(fromIntro){
  ...
  if(fromIntro && !REDUCED_MOTION){
    ...
    for(let n=10;n>=1;n--){ showMsg(String(n),"count"); beep(n===1?880:440,0.16); await wait(520);
...
function altClimb(){
  ...
  altTimer=setInterval(()=>{ altitude+=Math.random()*4+2;
    hudAlt.textContent=altitude.toFixed(1).padStart(5,"0"); },120);
}
...
function openWindow(st){
  ...
  typeText(line,st.type,()=>{
    ...
    else if(st.extra){ const ex=document.createElement("div"); ex.className="reveal"; ex.innerHTML=st.extra;
      winBody.appendChild(ex); requestAnimationFrame(()=>ex.classList.add("in")); }
  });
}
function typeText(el,text,done){
  let i=0; clearInterval(typing);
  typing=setInterval(()=>{ el.textContent=text.slice(0,++i); winBody.scrollTop=winBody.scrollHeight;
    if(i>=text.length){clearInterval(typing); if(done)done();} },16);
}
